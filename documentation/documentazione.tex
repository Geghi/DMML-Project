...
The result is printed into a graph which shows the number of tweet analyzed so far and the semtiment rating. The graph keeps updating until the user clicks the stop button or when the application is closed. 
\vspace{5mm}
\begin{figure}[h]
\centering
...
\end{itemize}
The system has to continuously perform the following operations: 
\begin{itemize}
